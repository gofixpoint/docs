...
Resolve every URL in the message against the docs tree. For internal doc links, confirm the page exists and any `#anchor` matches a real heading. For external links (app.amika.dev, GitHub, Discord), check they at least look well-formed and consistent — flag anything that looks like it points to two different targets for the same thing (e.g. two different API-doc URLs for what should be one canonical page).
