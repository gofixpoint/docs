...
When you run `amika rig create --git` and the repository contains service declarations, Amika resolves host ports for each service port:
...
Resolved ports and URLs are visible in `amika rig list` and API responses.
...
Use `amika service list` to see services across all rigs:
