...
Defines a graph of prompt steps the agent runs in sequence, branching on each step's success or failure.
...
# both transitions omitted -> any outcome is terminal
```
...
| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `start` | string (step name) | Yes | The entry step. Must reference a defined step. |
| `default_agent` | `"claude"` or `"codex"` | No | Agent used to run the workflow. |
| `steps` | table of `[workflow.steps.<name>]` | Yes | At least one step must be defined. |

### Step fields

Each `[workflow.steps.<name>]` table accepts:

| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `prompt_file` | string (repo-relative path) | Yes | Path to the step's prompt. |
| `on_success` | string (step name) | No | Next step on success. Omit to mark terminal-success. |
| `on_failure` | string (step name) | No | Next step on failure. Omit to mark terminal-failure. |

### Validation rules

| Rule | Detail |
| --- | --- |
| **Step name** | Must match `^[a-zA-Z0-9_][a-zA-Z0-9_-]{0,63}$`. |
| **References** | `start`, `on_success`, and `on_failure` must each name a defined step. |
| **`prompt_file`** | Must be repo-relative — no leading `/` and no `..`. Existence is checked at sandbox-create time when the repo is cloned. |
| **At least one step** | `[workflow.steps]` must define at least one step. |

If the `[workflow]` block is present but malformed, the create reports the validation error.
