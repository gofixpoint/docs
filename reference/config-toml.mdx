...
For an overview of how repository configuration works and how it interacts with other configuration sources, see [Repository configuration](/guides/repository-configuration).
...
- `"port"` — the host-side port the service is exposed on.
...
Declares named services with port bindings and optional URL schemes. Each `[services.<name>]` section defines one service. See [Services](/guides/services) for usage examples and runtime behavior.
...
See [Presets](/guides/presets) for full details on what each preset includes.
...
- [Repository configuration](/guides/repository-configuration) — how repo config works and resolution precedence
- [Sandbox configuration](/guides/sandbox-configuration) — CLI flags and runtime settings
- [Services](/guides/services) — service runtime behavior, port resolution, and listing
