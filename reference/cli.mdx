...
Manage Docker-backed persistent rigs with bind mounts and named volumes.
...
These flags apply to all `rig` subcommands (`create`, `list`, `connect`, `stop`, `start`, `delete`, `ssh`, `code`, `agent-send`):

| Flag | Default | Description |
| --- | --- | --- |
| `--local` | `false` | Only operate on local rigs |
| `--remote` | `false` | Only operate on remote rigs |

When neither flag is set, commands default to remote mode. Use `--local` to operate on local rigs only.
...
amika rig create --name dev-rig --port 8080:8080

# Publish a port bound to all interfaces
amika rig create --name dev-rig --port 3000:3000 --port-host-ip 0.0.0.0

# Clone a specific git branch
amika rig create --name dev-rig --git --branch develop

# Create a new branch from your current branch
amika rig create --git --new-branch feature-x

# Create a new branch starting from a specific existing branch
amika rig create --git --branch main --new-branch bugfix-1

# Inject remote secrets (remote rigs only)
amika rig create --name dev-rig --git --remote \
...
| `--provider <name>` | `docker` | Compute provider (only `docker` is currently supported) |
...
| `--git [path-or-URL]` | | Mount a git repo (accepts a local path or a git URL over HTTPS or SSH) to `/home/amika/workspace/{repo}`. If omitted and the cwd is inside a git repo, that repo is used automatically |
...
| `--size <size>` | `m` | Rig resource size: `xs` or `m`. Remote rigs only |
...
amika service list
amika service ls
amika service list --sandbox-name dev-rig
```
...
| Flag | Default | Description |
| --- | --- | --- |
| `--sandbox-name <name>` | (none) | Filter services to a specific rig |
