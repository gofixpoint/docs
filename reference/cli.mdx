...
| `--no-git` | `false` | Skip git repo auto-detection; create a repo-less sandbox with no repo mounted |
...
Manage snapshots captured from running sandboxes. Snapshots are a hosted-cloud
...
Capture a snapshot from a running sandbox. Interactive by default: it prompts for
...
| Flag | Default | Description |
| --- | --- | --- |
| `--sandbox <name-or-id>` | **(required)** | Source sandbox to snapshot |
...
| Mode | Behavior |
| --- | --- |
| `scrub_and_delete` (default) | Remove Amika-injected secrets, capture the snapshot, then delete the source sandbox |
| `full` | Capture everything as-is (keeps secrets and the source sandbox; unsafe for shared images) |
...
| `--sandbox <name-or-id>` | | Only show snapshots captured from this sandbox |
