...
To skip the browser flow and log in with a WorkOS organization API key — useful in CI — pass `--api-key-file`. Use `-` to read from stdin so the key never lands on disk:
...
| Flag | Default | Description |
| --- | --- | --- |
| `--api-key-file <path \| ->` | | Read a WorkOS organization API key from this path (or `-` for stdin) instead of running the device flow |
