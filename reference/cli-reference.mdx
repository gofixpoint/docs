...
<Note>
Supported agent kinds for `--agent-credential`, `--agent-credential-type`, and `--no-agent-credential` are `claude` and `codex`. Each flag may appear at most once per kind. `--no-agent-credential` is mutually exclusive with the other two for the same kind. See [Pin a credential at sandbox creation](/guides/agent-authentication#pin-a-credential-at-sandbox-creation) for usage.
</Note>
