...
- Tools: Everything in `coder`, plus Docker-in-Docker (dind) support for
  running containerized services inside the rig
- Agent CLIs: Claude Code, Codex, OpenCode
...
Use `--setup-script` to inject your setup logic when creating a rig:

```bash
amika rig create --setup-script ./install-deps.sh
```

See [Rig configuration](/reference/rig-configuration) for details.
...
Preset images reserve container ports **60899–60999** for Amika internal services (e.g. OpenCode web on 60998, amikad daemon on 60999). See [Rig configuration](/reference/rig-configuration#reserved-ports) for the full allocation table.
...
When you create a rig or run materialize, Amika auto-discovers and mounts supported credential files into the container.

This gives agents credentials inside the rig without modifying host source files. See [Rig configuration](/reference/rig-configuration#agent-credential-auto-mounting) for the full list of supported files.
