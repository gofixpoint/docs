...
channel or DM. Mention the bot with a request and Amika spins up a rig for
that thread, runs the agent, and replies in-thread. Follow-up messages in the
same thread continue the same rig and session.
...
Each Slack thread maps to its own rig. Continuing the thread continues that
rig's session, so the agent keeps its context across follow-up messages.
