...
start_script = ".amika/scripts/start.sh"  # optional, runs on every start
...
- `setup_script` is a path relative to the repo root pointing to an executable script that runs once before the agent starts.
- `start_script` (optional) is an executable script that runs on **every** sandbox start, after `setup_script` on create and again each time a stopped sandbox restarts. Put one-time provisioning in `setup_script`; put services that must come back up on every boot in `start_script`.
...
**Where to start services:** if you defined a `start_script`, put service startup (Phase 4) there instead of in the setup script. The setup script runs only once at create, but the start script runs on every boot, so services started there come back up when a stopped sandbox restarts. Keep one-time install/build steps (Phases 1–3) in the setup script. If you are only generating a setup script, backgrounding services in Phase 4 is fine.
...
- `start_script` (string, optional): Path to an executable script that runs on every sandbox start — after `setup_script` on create, and again each time a stopped sandbox restarts. Use it for services that must come back up on every boot.
