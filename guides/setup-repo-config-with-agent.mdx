...
### Preset selection
Explain the available Amika presets and recommend one:
...
- **`coder-dind`** — Same base tools as `coder`, but also configures Docker in Docker (dind) to allow running containerized services.

Both presets use the same Ubuntu 24.04 base and development tools. The only difference is which AI agent CLIs are preinstalled. Recommend `coder` unless the user requires Docker or containerization.

Note: The preset is selected at sandbox creation time (`amika sandbox create --preset coder`), not in config.toml. But the user should know which preset to use since it determines what's pre-installed and affects what the setup script needs to install.
...
### Missing runtimes
Based on the preset's pre-installed tools (Python, Node.js 22, pnpm, TypeScript), identify what's missing that the setup script will need to install. For example:
...
This is the main configuration file. It has three sections:
...
```bash
amika sandbox create --git --preset coder
```
...
Presets are pre-built Docker images for sandboxes. Selected at sandbox creation time, not in config.toml.

| Preset | Image | Agent CLIs | Pre-installed tools |
|--------|-------|------------|---------------------|
| `coder` (default) | `amika/coder:latest` | Claude Code, Codex, OpenCode | Ubuntu 24.04, Git, curl, zsh, build essentials, Python, Node.js 22, pnpm, TypeScript, tsx |
| `claude` | `amika/claude:latest` | Claude Code only | Same as coder |
...
```bash
# Clone current repo into a sandbox (clean checkout)
amika sandbox create --git --preset coder

# Include uncommitted/untracked files
amika sandbox create --git --no-clean --preset coder

# Clone a specific branch
amika sandbox create --git --branch my-branch --preset coder
