...
description: "Use a coding agent to analyze your repository and generate an Amika rig configuration automatically."
---
...
Create a rig loaded with the repo you are setting up.
...
Or, go to https://app.amika.dev/sandbox and create a rig for your repo, then connect to it.

In the rig, navigate to the git repo, and copy the prompt below into your coding agent. The agent will:
...
3. **Generate** a `.amika/config.toml` and `.amika/scripts/setup.sh` ready for rig creation.
...
You are tasked with setting up an Amika rig configuration for this repository. Amika is infrastructure for building a software factory — it spins up sandboxed development environments pre-loaded with coding agents and your repository code. Your job is to create a `.amika/config.toml` and a setup script so that when a rig is created from this repo, it has all dependencies installed and dev services running.

## Reference materials

Before you begin, read these resources to understand how Amika configuration works:

1. **Amika docs — rig configuration**: https://docs.amika.dev/reference/rig-configuration
...
### Rig settings
Explain the available rig presets and sizes, and recommend values:
...
- **`coder-dind`** — Everything in `coder`, plus Docker-in-Docker (dind) support for running containerized services inside the rig.
...
These values are set in the `[sandbox]` section of `.amika/config.toml` and used as defaults when creating a rig from this repository. They can be overridden at creation time via CLI flags or the web UI.

### Services
List the services you discovered and the ports they use. Ask the user:
- Which of these services should be exposed in the rig?
...
- **Required secrets** — values the user must provide via `amika secret push` or the web UI before creating a rig
...
- For database/service connection strings, should these point to a shared dev instance, or will they run locally in the rig?
...
- `setup_script` is a path relative to the repo root pointing to an executable script that runs once when a new rig is created, before the agent starts.
- `start_script` (optional) is an executable script that runs each time a stopped rig is started again — not on the initial create, where only `setup_script` runs. Put one-time provisioning in `setup_script`; use `start_script` for steps that must repeat whenever a rig resumes.
- Services expose ports from the rig. Each service has a name, port, and optional `url_scheme`. Only set `url_scheme` for HTTP/HTTPS services — omit it for raw TCP services like gRPC.
- Ports 60899–60999 are reserved by Amika — do not use them.
- Environment variables can be plain strings or secret references. Secrets must be pushed via `amika secret push` or the Amika web UI before rig creation. Use `{ secret = "name" }` syntax for secrets.

### `.amika/scripts/setup.sh`

This script runs inside the rig before the agent starts. It must:
1. Be executable (`chmod +x`)
2. Use `$AMIKA_AGENT_CWD` to reference the repo root (do NOT hardcode paths)
3. Exit 0 on success (non-zero aborts rig startup)
...
**Where to start services:** the setup script runs once, on create; the start script runs each time a stopped rig is restarted, but not on the initial create. Start services in the setup script so they come up on a fresh rig. If the project will be stopped and resumed, repeat that service startup (Phase 4) in the start script so the services come back. Keep one-time install/build steps (Phases 1–3) in the setup script only.
...
Only generate this if you set `start_script` in `config.toml`. It runs each time a stopped rig is resumed — not on the initial create — so it must bring services back up after a restart. It has the same requirements as the setup script: executable (`chmod +x`), use `$AMIKA_AGENT_CWD` for the repo root, exit 0 on success.
...
Tell the user what secrets they need to push before creating a rig:
```bash
amika secret push <SECRET_NAME>=<value>
```

And how to create a rig with this config (run from inside the repo; the repo is auto-detected from the current directory):
```bash
amika rig create
```
...
**`[sandbox]`** — Default rig settings for the repository.
...
These values are used as defaults when creating a rig from this repository. They can be overridden at creation time via CLI flags or the web UI.

**`[lifecycle]`** — Rig lifecycle hooks.
...
- `start_script` (string, optional): Path to an executable script that runs each time a stopped rig is restarted — not on the initial create, where only `setup_script` runs. Use it to bring services back up when a rig resumes.

**`[filesystem]`** — Additional repos to clone alongside the primary repo.
- `repos` (list of strings): GitHub repo URLs to clone into the rig at `~/workspace/<repo-name>`. The primary repo is inferred automatically and need not be listed. Only set this if the project spans multiple repositories.
...
**`[env]`** — Environment variables injected into the rig.
- Plain values: `DATABASE_HOST = "localhost"`
- Secret references: `API_KEY = { secret = "my-api-key" }` — the secret must be pushed to the remote store before rig creation.
...
Presets are pre-built Docker images for rigs. Set via the `[sandbox]` section in config.toml or overridden at creation time with `--preset`.
...
- `$AMIKA_AGENT_CWD` — the path to the cloned repository inside the rig (typically `/home/amika/workspace/<repo-name>`)
- Credentials for coding agents are auto-mounted from the host
- The script runs as the `amika` user with `sudo` access
- If the script exits non-zero, the rig does not start
...
```bash
# Clone current repo into a rig (auto-detected from cwd; uses preset and size from config.toml).
# To target a different repo, pass a path or git URL: amika rig create --git <path-or-url>
amika rig create

# Override preset at creation time
amika rig create --preset coder-dind

# Include uncommitted/untracked files (local rigs only)
amika rig create --no-clean

# Clone a specific branch
amika rig create --branch my-branch
...
1. **Commit the `.amika/` files** to the repository. `amika rig create` clones a clean copy of the committed tree by default, so uncommitted config and scripts are not picked up:

```bash
git add .amika && git commit -m "Add Amika rig configuration"
```
...
3. **Create a rig** to test the configuration (run from inside the repo; the repo is auto-detected):
