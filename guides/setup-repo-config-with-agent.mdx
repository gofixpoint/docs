...
Copy the prompt below into your coding agent while inside your repository. The agent will:
...
- **`claude`** — Same base tools as `coder`, but only includes Claude Code (no Codex or OpenCode).

Both presets use the same Ubuntu 24.04 base and development tools. The only difference is which AI agent CLIs are preinstalled. Recommend `coder` unless the user only uses Claude Code.
