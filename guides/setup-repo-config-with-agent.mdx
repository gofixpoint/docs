...
You are tasked with setting up an Amika rig configuration for this repository. Amika is infrastructure for building a software factory — it spins up isolated development environments pre-loaded with coding agents and your repository code. Your job is to create a `.amika/config.toml` and a setup script so that when a rig is created from this repo, it has all dependencies installed and dev services running.
