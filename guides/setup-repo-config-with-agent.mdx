...
- Service references: `WEB_URL = { service = "web", field = "url" }` — injects another service's generated `url` or published `port`. `field` is `"url"` or `"port"`; `"url"` requires that service to have a `url_scheme`.
