...
MCP (Model Context Protocol) integrations let a sandboxed coding agent call out to external services — error trackers, project management tools, internal systems — without running an interactive OAuth flow inside the rig.

You connect your account to a provider once on the host. Amika stores the OAuth token in the secrets vault, attaches it to each rig created from repositories that use the integration, and writes the MCP server configuration into the rig's `~/.claude/.config.json` so Claude Code picks it up on startup.
...
Integrations attach to repositories, so rigs inherit the right tools automatically.
...
The **Create rig** dialog pre-checks every integration attached to the repo. Uncheck any you don't want for that rig, and Amika skips them at provisioning time. The final list of integrations is recorded on the rig so you can audit which tools the agent had access to.
...
This is the same location `claude mcp add` writes to, so Claude Code trusts the servers implicitly — no in-rig approval prompt.
...
- **Token refresh is manual.** If a provider's access token expires while a rig is running, MCP requests start returning 401. Reconnect the provider on `/integrations` and recreate the rig.
