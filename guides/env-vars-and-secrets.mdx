...
Every value you inject into a rig is one of two things: a **plain
...
<Warning>
Secrets are resolved at rig creation time and **copied into the rig VM**,
where they are readable as environment variables from inside the rig. The
vault protects secrets at rest and keeps them out of your repository — it does
not currently isolate them from code running in the rig.
</Warning>
...
ships, treat any secret injected into a rig as visible to the agent and any
code it runs.

## Where to go next

- [Manage secrets](/guides/manage-secrets) — push, list, and update vault secrets, including Claude and Codex credentials.
- [Inject credentials](/guides/inject-credentials) — wire stored secrets and agent credentials into a rig at create time.
