...
description: "Script the Amika CLI to spin up a rig, drive an agent non-interactively, and ship the result."
---
...
push a credential, create a remote rig cloned from a git repo, drive Claude
Code non-interactively to make a change and open a PR, then tear the rig
down.
...
<Info>
The pattern below uses `amika rig ssh` with heredocs to control the agent.
...
<Step title="Create a remote rig cloned from the repo">
...
Running the command inside a Git repository selects that repository.
`--agent-credential-type claude=api-key` tells Amika to inject your stored
API-key credential, which avoids the OAuth picker.

</Step>

<Step title="Configure agent permissions inside the rig">

Use a heredoc with `amika rig ssh` to write a Claude
`settings.local.json`. The `$AMIKA_AGENT_CWD` env var resolves to the cloned
repo root inside the rig.
...
amika rig ssh testing-claude-api << EOF
...
escaped with `\$` so they survive to the rig shell and resolve there.
...
amika rig ssh -t testing-claude-api << EOF
...
<Step title="Tear down the rig">

```bash
amika rig rm --force testing-claude-api
```
...
errcho "Configuring Claude settings in rig..."
amika rig ssh testing-claude-api << EOF
mkdir -p \$AMIKA_AGENT_CWD/.claude
echo "settings.local.json" > \$AMIKA_AGENT_CWD/.claude/.gitignore
echo '$CLAUDE_SETTINGS' > \$AMIKA_AGENT_CWD/.claude/settings.local.json
...
so a single failed run does not leak a rig. `amika rig rm --force`
...
<CardGroup cols={2}>
  <Card title="Inject credentials" icon="key-round" href="/guides/inject-credentials">
    Push and pin Claude or Codex credentials for rigs.
  </Card>
  <Card title="CLI reference" icon="square-terminal" href="/reference/cli">
    Full flag listings for rig, secret, and ssh commands.
  </Card>
</CardGroup>
