...
Events are written under the amika state directory (`~/.local/state/amika` by
default; override with `AMIKA_STATE_DIRECTORY`):
...
Local capture works entirely offline. To pool your team's sessions in one
...
`<repo>/<source>/sessions/<ts>_<session-id>.jsonl`. `<repo>` is the lowercased
**basename** of the session's `git.repo_root`, sanitized so path separators and
a few reserved characters become `-`, and falls back to `unknown-repo` when no
event carries git context — so a repo at `/work/acme/API` lands under `api/...`.
Pushed files are tracked by size, so repeated runs re-upload only sessions that
grew new events, and re-pushing is idempotent.
