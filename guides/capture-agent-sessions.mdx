...
`<repo>/<source>/sessions/<ts>_<session-id>.jsonl`, where `<repo>` comes from
the session's `git.repo_root`. Pushed files are tracked by size, so repeated
runs re-upload only sessions that grew new events, and re-pushing is
idempotent.
