...
description: "Wire stored secrets and agent credentials into a rig at create time."
---
...
injected into each new rig. Amika supports three patterns:
...
Map a rig env var to a secret stored in the vault.

<CodeGroup>

```bash CLI
# Map a specific env var to a secret by name
amika rig create --name dev-box \
  --secret env:ANTHROPIC_API_KEY=my-claude-key

# Shorthand: env var name matches the secret name
amika rig create --name dev-box --secret env:DATABASE_URL
```

```ts TypeScript
await amika.createRig({
  name: "dev-box",
  secretEnvVars: {
...
The rig response includes a `resolvedAgentCredentials` field that shows,
per kind, the outcome (`resolved` or `skipped`), the source, and a reason.
Use it to confirm the right credential made it into the rig.

```ts
const sb = await amika.getRig("dev-box");
...
The **New rig** form on [app.amika.dev](https://app.amika.dev) lets you
