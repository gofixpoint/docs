...
comments, so they live in source and travel with the code in git.
...
This writes a single-use override token to `.amika/state/` that allows
the next commit on this branch to modify `SCHEMA_VERSION`. After that
commit lands the override is consumed — subsequent commits will be
blocked again, and you'll need to re-run the override or remove the
annotation. The token is per-branch state, not per-terminal, so it
survives across sessions until the commit consumes it.
