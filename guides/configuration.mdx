...
description: "How a repository configures its rigs and agents, and how Amika resolves settings across sources."
---

A rig needs to know what image to boot, what to install, what services to
...
When a rig is created from the repo, Amika reads the file and applies the
...
This guide explains how configuration reaches a rig: where it can come from,
how Amika resolves conflicts, and how it behaves for multi-repo and repo-less
rigs. For the full field-by-field spec, see
[config.toml reference](/guides/config-toml).

## What a repo config controls

`.amika/config.toml` is a template for the rigs created from a repository.
Each section maps to one dimension of the rig:
...
- **`[workflow]`**: a hosted multi-step agent workflow.
...
| --- | --- | --- |
| 1 (highest) | **API request / CLI flags** | Values passed at rig creation time |
...
and a size from a UI setting can all apply to the same rig at once. A value
saved through the web UI becomes the "Database" layer and overrides the
corresponding `.amika/config.toml` value for all future rigs; the UI shows
...
Setting the rig preset is a good example — you can declare it once in the
repo, override it per rig from the CLI, or set a repository default in the
UI. All three feed the resolution table above.

<Tabs defaultTabIndex={0}>
  <Tab title=".amika/config.toml">
    Commit the default with your code. It applies to every rig created from
    the repo.
  ...
  <Tab title="CLI">
    Override the preset for a single rig at create time.
...
A rig can clone more than one repository. List the extra repos under
...
the one the rig is created from — the repo that carries the `[filesystem]`
...
everything the rig needs in the primary repo's config.
</Warning>
...
A repository is optional. A rig can boot with **no repo** — and therefore no
...
```bash
# Create a repo-less rig and configure it inline
amika rig create --name scratch --no-git --preset coder --size m
```

See [Create a rig](/guides/create-a-rig) for how the CLI picks (or
skips) a repo.

## Related docs

- [config.toml reference](/guides/config-toml) — full field-by-field spec
- [Create a rig](/guides/create-a-rig) — repo detection and `--no-git`
